...
class GeneralKNNFusionModule(nn.Module):
    def __init__(self, in_channels, out_channels_list_before, out_channels_list_after,
        ...
        # Channel attention: max over K per channel, then SE gate on C (USIP-yuan is max-only; this branch is retained).
        c_red = max(previous_out_channels // 8, 8)
        self.c_attn_conv1 = nn.Conv2d(previous_out_channels, c_red, kernel_size=1, bias=True)
        self.c_attn_conv2 = nn.Conv2d(c_red, previous_out_channels, kernel_size=1, bias=True)
    ...
    def forward(self, query, database, x, K, epoch=None):
        ...
        c_pool, _ = torch.max(query_neighbor, dim=3, keepdim=True)
        c_gate = torch.sigmoid(
            self.c_attn_conv2(F.relu(self.c_attn_conv1(c_pool)))
        )
        query_neighbor = query_neighbor * c_gate
