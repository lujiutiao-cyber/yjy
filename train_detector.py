...
if __name__=='__main__':
    ...
    for epoch in range(301):
        ...
        for i, data in enumerate(trainloader):
            ...
            if i % int(32 / opt.batch_size * 80) == 0 and i > 0:
                ...
                with open("./loss_log.txt", "a") as f:
                    f.write(
                        f"epoch:{epoch} iter:{epoch_iter} loss:{errors['O_loss']} chamfer:{errors['O_chamfer']} key_on_pc:{errors['O_key_on_pc']}\n")
                        #f"epoch:{epoch} iter:{epoch_iter} loss:{errors['Loss']} chamfer:{errors['chamfer']} key_on_pc:{errors['key_on_pc']}\n")
        ...
        #下面是测试集代码：没有单独的 test.py
        #训练时每一轮 Epoch 结束，自动跑一次测试集
        #看到的每一个 .pth 文件后面的 loss，就是测试集的 loss
