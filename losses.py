...
class ChamferLoss_Brute(nn.Module):
    def __init__(self, opt):
        ...
        self.opt = opt
        self.dimension = 3
        # Optional dynamic overlap weighting (paper-inspired).
        # Keep defaults backward-compatible when options are absent.
        self.use_dynamic_overlap_weight = bool(getattr(opt, 'use_dynamic_overlap_weight', True))
        self.dynamic_overlap_eps = float(getattr(opt, 'dynamic_overlap_eps', 1e-6))

    def _dynamic_overlap_coeff(self, min_dist):
        """
        min_dist: BxM nearest-neighbor distance tensor.
        gamma_b = M / count(d < mean(d)+std(d)), clamped for stability.
        """
        if not self.use_dynamic_overlap_weight:
            return torch.ones(min_dist.size(0), device=min_dist.device, dtype=min_dist.dtype)

        mean_d = torch.mean(min_dist, dim=1, keepdim=True)  # Bx1
        std_d = torch.std(min_dist, dim=1, keepdim=True, unbiased=False)  # Bx1
        thr = mean_d + std_d
        inlier_num = torch.sum((min_dist < thr).float(), dim=1)  # B
        inlier_num = torch.clamp(inlier_num, min=1.0)
        gamma = (min_dist.size(1) / (inlier_num + self.dynamic_overlap_eps)).detach()  # B
        return gamma
    ...
    def forward(self, pc_src_input, pc_dst_input, sigma_src=None, sigma_dst=None):
        ...
        if sigma_src is None or sigma_dst is None:
            # pc_src vs selected pc_dst, M
            src_dst_min_dist, _ = torch.min(diff, dim=2, keepdim=False)  # BxM
            gamma_src = self._dynamic_overlap_coeff(src_dst_min_dist)  # B
            forward_loss = (src_dst_min_dist * gamma_src.unsqueeze(1)).mean()

            # pc_dst vs selected pc_src, N
            dst_src_min_dist, _ = torch.min(diff, dim=1, keepdim=False)  # BxN
            gamma_dst = self._dynamic_overlap_coeff(dst_src_min_dist)  # B
            backward_loss = (dst_src_min_dist * gamma_dst.unsqueeze(1)).mean()

            chamfer_pure = (src_dst_min_dist.mean() + dst_src_min_dist.mean()).detach()
            chamfer_weighted = (forward_loss + backward_loss).detach()
        else:
            # pc_src vs selected pc_dst, M
            ...
            gamma_src = self._dynamic_overlap_coeff(src_dst_min_dist)  # B
            forward_term = torch.log(sigma_src_dst) + src_dst_min_dist / sigma_src_dst  # BxM
            forward_loss = (forward_term * gamma_src.unsqueeze(1)).mean()
            ...
            gamma_dst = self._dynamic_overlap_coeff(dst_src_min_dist)  # B
            backward_term = torch.log(sigma_dst_src) + dst_src_min_dist / sigma_dst_src  # BxN
            backward_loss = (backward_term * gamma_dst.unsqueeze(1)).mean()
